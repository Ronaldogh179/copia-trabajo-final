...
class TestSmartTaskDB(unittest.TestCase):
    ...
    def test_crear_tarea_exitosamente(self):
        ...
        tarea_id = self.db.crear_tarea(
            "Estudiar Python", "Para el examen", "2026-02-28", "alta"
        )

        # Validación de Resultados (8.2)
        self.assertIsNotNone(
            tarea_id, "El ID de la tarea no debería ser None (Fallo de persistencia)"
        )
        ...
        self.assertEqual(
            tarea.titulo,
            "Estudiar Python",
            "El título guardado no coincide con el enviado",
        )
        session.close()
    ...
    def test_impedir_titulo_vacio(self):
        ...
        self.assertIsNone(
            tarea_id, "El sistema debería haber rechazado un título vacío"
        )
    ...
    def test_eliminar_tarea(self):
        ...
        self.assertIsNone(
            tarea, "La tarea debería haber desaparecido de la BD tras eliminarla"
        )
        session.close()
